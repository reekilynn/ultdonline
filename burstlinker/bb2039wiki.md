...
- 这一部分内容对1vs1的通常对战进行说明。

- Burst Linker之间的《**对战**（Duel）》和旧时代的《对战格斗游戏》一样，双方互相削减HP，在其中一方HP归零的时候决出胜负。

- 对战开始时，使用《Burst Link》命令加速，在对战玩家列表中选择想要对战的Burst Linker并按下《DUEL》按钮，亦可将Neurolinker连接到广域网或局域网（即待机状态）等待他人的《乱入》。换而言之，通常对战必须在连接到广域网或局域网的情况下才能进行。

- 对战时间为30分=1800秒，即现实世界的1.8秒。

- 遭受伤害、造成伤害或破坏地图内可破坏的物品时均可增加必杀技值。消耗必杀技值可以使用必杀技或能力。

- 通常对战的区域面积等于对战所在的战区面积。

- 对战双方的距离超过10米，处于各自视野外时，会在视野中央出现《导航光标》。这一水蓝色的小三角形会指示对手所在的方向，但无法显示距离。

- 持有强化外装的情况下，通过装备命令（默认为《｛道具名｝装备》，可在指示界面中变更）的发声进行装备或解除装备。

- 战斗时间截止（1800秒）时双方均仍有HP的情况下，HP较多的一方取胜。HP相等时判定为平局，不发生BP的移动。

- 通过在BB控制台添加想要观战的对战虚拟体名，可在观战对象开始对战时自动加速（不消耗BP）进行观战。

- 观战者不能进入对战者半径十米范围内的空间，但身为对战者的《亲》或同军团成员的情况不受限制。

- 在指示界面中按下《平局申请》的按钮，对方同意的情况下，该场对战算作平局。

- 在指示界面中按下《封闭模式申请》的按钮，对方同意的情况下，可将对战区域设为《封闭模式》，禁止他人观战。

- 在指示界面中按下《选择性排除观战者》的按钮，选择想要排除的观战者，对方同意的情况下，可将任意观战者从对战中排除。
...
- 《指示界面》是《指示卡片界面（Instruction Card）》这一正式名称的省略形式，指的是Brain Burst中执行各项操作的界面，等同于其他在线游戏的主菜单或状态窗口等界面。

- 打开指示界面的方法有如下几个：

  1. 通过《Burst Link》命令加速进入《初始加速空间》，点击虚拟桌面上的Brain Burst图标启动程序。在非加速状态下会打开BB控制台，但在初始加速空间下则会进入指示界面。但这一操作需要Neurolinker处于在线状态（操作指示界面时，Neurolinker会与Brain Burst中央服务器进行通信）。
  2. 通常对战时，触碰显示在视野左上方的，自己的HP槽或对战虚拟体名称。
  3. 观战时，触碰对战者任意一人的体力值或对战虚拟体名称（此时打开的仍然是自己的指示界面）。

- 指示界面上默认显示五个标签栏：
  1. 状态（Status）：自身的对战虚拟体名称、等级、所属军团、持有强化外装、持有能力、已习得的必杀技、外观轮廓。
  2. 对战玩家列表：连接到所在战区或当前局域网下的对战虚拟体列表。只有在从初始加速空间打开指示界面的情况下，才能对任意对手提出对战（通称《乱入》）。
  3. 对战选项：更改正在进行的对战模式或驱逐会产生妨害作用的观战者。在初始加速空间中或在观战时，无法进行这些操作。
  4. 持有道具栏（Storage）：显示目前持有的道具（包含封印中的强化外装）。但几乎所有的道具都只能在无限制中立空间中获得或使用。
  5. 点数操作：确认持有点数、变动历史、对战胜率等内容，也可消费点数进行升级。

- 加入军团或成为军团长的情况下，会在指示界面显示名为《军团》的第六个标签。
...
- 防守一方于领土战开始前，需在指示界面的军团选项卡中指定防守每一部分领土的成员，并令他们在领土内准备。这些成员在领土战期间会被自动从对战玩家列表中屏蔽出去。防守方的成员人数可自由决定，不设上限。此外，防守方的成员也不限制出击次数。

- 攻击一方于领土战开始前，需在指示界面的军团选项卡中指定攻击每一部分领土的成员，并令他们在领土内准备。与防守方不同，他们不会被从对战玩家列表中评比出去，因此一般会在攻击开始前才连接到广域网。攻击方在领土战中查看对战列表时，会出现防御方军团的名字，点击军团名后按下DUEL按钮开始领土战。但是，攻击方的人数不得超过防守方的人数，超过时按照等级从高到低的顺序自动选择（等级相等时随机选择）。对同一区域的领土战，攻击方每人只能出击一次。

- 作为例外，防御方的队伍只有1到2人的情况时，攻击方可以设定3人发动攻击。由于这一机制的存在，军团的成员需要至少三人。

- 领土战的区域中包含了数个通常对战区域中没有的《**据点**（Foot Hold）》。在据点内，对战虚拟体的必杀技值会自动恢复。

- 领土战的胜者，为全灭敌方队伍的一方、30分钟后存活人数更多的一方、或存活人数相同时HP值总量更多的一方。战斗结束后，系统会自动评判双方的战斗表现，并以《评价点》的形式显示。
